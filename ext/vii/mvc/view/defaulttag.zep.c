...
ZEPHIR_INIT_CLASS(Vii_Mvc_View_DefaultTag) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Vii\\Mvc\\View, DefaultTag, vii, mvc_view_defaulttag, vii_mvc_view_taglibarys_ce, vii_mvc_view_defaulttag_method_entry, 0);

	zend_declare_property_null(vii_mvc_view_defaulttag_ce, SL("tags"), ZEND_ACC_PUBLIC TSRMLS_CC);

	vii_mvc_view_defaulttag_ce->create_object = zephir_init_properties_Vii_Mvc_View_DefaultTag;
	return SUCCESS;
...
PHP_METHOD(Vii_Mvc_View_DefaultTag, _if) {
	...
	if (zephir_array_isset_string(params, SS("condition"))) {
		...
		zephir_array_fetch_string(&condition, params, SL("condition"), PH_NOISY, "vii/mvc/view/defaulttag.zep", 23 TSRMLS_CC);
	} else {
		ZEPHIR_INIT_NVAR(condition);
		ZVAL_BOOL(condition, 0);
	}
	if (zephir_is_true(condition)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "if标签内必须有condition属性", "vii/mvc/view/defaulttag.zep", 25);
		return;
	}
	ZEPHIR_INIT_VAR(str);
...
PHP_METHOD(Vii_Mvc_View_DefaultTag, _js) {
	...
	zephir_array_fetch_string(&_0, params, SL("src"), PH_NOISY | PH_READONLY, "vii/mvc/view/defaulttag.zep", 38 TSRMLS_CC);
...
PHP_METHOD(Vii_Mvc_View_DefaultTag, _css) {
	...
	zephir_array_fetch_string(&_0, params, SL("link"), PH_NOISY | PH_READONLY, "vii/mvc/view/defaulttag.zep", 43 TSRMLS_CC);
...
PHP_METHOD(Vii_Mvc_View_DefaultTag, _list) {
	...
	zephir_array_fetch_string(&_0, params, SL("data"), PH_NOISY | PH_READONLY, "vii/mvc/view/defaulttag.zep", 54 TSRMLS_CC);
	zephir_array_fetch_string(&_1, params, SL("item"), PH_NOISY | PH_READONLY, "vii/mvc/view/defaulttag.zep", 54 TSRMLS_CC);
