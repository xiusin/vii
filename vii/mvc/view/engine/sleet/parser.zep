...
namespace Vii\Mvc\View\Engine\Sleet;
...
class Parser
{
            ...
            switch ch {
                case '{':
                    ...
                    if end === false {
                        ...
                        throw new ViewException(sprintf("Unclosed echo on the line %d", substr_count(substr(text, 0, start), PHP_EOL) + 1));
                    }
                ...
                case '%':
                    ...
                    if end === false {
                        // If unexpected end of template
                        throw new ViewException(sprintf("Unclosed tag on the line %d", substr_count(substr(text, 0, start), PHP_EOL) + 1));
                    }
                ...
                case '#':
                    ...
                    if end === false {
                        // If unexpected end of template
                        throw new ViewException(sprintf("Unclosed comment block on the line %d", substr_count(substr(text, 0, start), PHP_EOL) + 1));
                    }
    ...
    public function parse(string expression) -> string
    {
        ...
        if typeof first != "array" {
            throw new ViewException("Unexpected first tag");
        }

        if typeof tokens != "array" {
            throw new ViewException("Unexpected expression");
        }
